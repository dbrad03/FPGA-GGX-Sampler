...
@cocotb.test()
async def test_inv_sqrt_nodsp(dut):
    """ Test axis_fixed_inv_sqrt_nodsp """
    # Setup clock & reset
    ...
    def checker_callback(transaction):
        if not sig_out_exp:
            assert False, "Unexpected output transaction"
        ...
        # We expect extremely high accuracy (e.g. < 0.0005) due to exact digit-recurrence
        TOLERANCE = 0.0005
        if diff > TOLERANCE:
            dut._log.error(f"Mismatch! Input: {in_x:.6f} | Act: {act_float:.7f} Exp: {exp_float:.7f} Diff: {diff:.7f}")
    ...
    # Mid-range and near-1
    midrange_vectors = [0.001, 0.01, 1/64, 1/16, 0.1, 0.25, 0.5, 0.75]
    near1_vectors = [0.9, 0.99, 0.999, np.nextafter(1.0, 0.0)]
    # Randoms
    random_vectors = []
    for _ in range(50):
        random_vectors.append(np.random.uniform(low=0, high=1.0))
    ...
    dut._log.info("Inv Sqrt No-DSP Test Passed!")
...
def inv_sqrt_runner():
    ...
    sources = sources_for("axis_fixed_inv_sqrt_nodsp")
    
    build_test_args = ["-Wall", "-I", str(proj_path / "hdl")]
    runner = get_runner(sim)
    hdl_toplevel = "axis_fixed_inv_sqrt_nodsp"
