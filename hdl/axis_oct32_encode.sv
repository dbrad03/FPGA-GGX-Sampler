...
module axis_oct32_encode #
  ...
  // The signs and TLAST ride INSIDE u_div_x, so they arrive at the field
  // mapping below out of the same output register as the quotient they belong
  // to. See the sideband note above.
  wire [3:0] div_sb_out;

  axis_fixed_div #(.WIDTH(DIV_W), .FRAC_BITS(DIV_F), .ELASTIC(0), .SIDEBAND_W(4)) u_div_x (
    ...
    .s00_axis_tuser({s2_last, s2_nz, s2_sy, s2_sx}),
    ...
    .m00_axis_tlast(), .m00_axis_tdata(qx_raw), .m00_axis_tuser(div_sb_out),
    .m00_axis_tstrb()
  );
  ...
  axis_fixed_div #(.WIDTH(DIV_W), .FRAC_BITS(DIV_F), .ELASTIC(0)) u_div_y (
    ...
    .m00_axis_tlast(), .m00_axis_tdata(qy_raw), .m00_axis_tuser(),
    .m00_axis_tstrb()
  );

  // The signs and TLAST come back out of u_div_x above, having travelled WITH
  // their own beat.
  //
  // They USED TO ride an exact-match delay line here, DIV_LAT deep, shifted on
  // front_en. That was correct only because the stallable divider gated its
  // whole pipeline with the same enable the line shifted on. A RIGID divider
  // (#33) advances every cycle whatever front_en does, and its ring then holds
  // the result for a variable number of cycles under backpressure, so nothing
  // of fixed depth outside it can track it. The failure mode is the silent one
  // -- a quotient paired with another Sample's signs, wrong by a reflection.
  //
  // The first attempt was an elastic FIFO here, pushed on the dividers' admit
  // and popped on their consume. Correct, and it cost 0.6 ns: its RAM read
  // landed in the same cycle as the field mapping's three carry chains
  // (sb_rd_ptr_reg -> m00_axis_tdata_reg, -1.120 measured). Carrying it inside
  // the core delivers it from the SAME output register as the quotient, so the
  // arithmetic below starts at a flip-flop -- and it deletes this block's
  // second copy of the divider's queueing state, which is the shape the enable
  // bug in 44cb724 was made of.
  wire d_sx   = div_sb_out[0];
  wire d_sy   = div_sb_out[1];
  wire d_nz   = div_sb_out[2];
  wire d_last = div_sb_out[3];
